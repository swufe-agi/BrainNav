...
def move_forward():
    limo.SetMotionCommand(1, -0.3)  # 直线前进
    time.sleep(1) 

def move_backward():
    limo.SetMotionCommand(0, 50)  
    time.sleep(1)  
    limo.SetMotionCommand(0, 1) 
    time.sleep(1) 
    limo.SetMotionCommand(0, 60)  
    time.sleep(1)  
    limo.SetMotionCommand(0, 1) 
    time.sleep(1) 
    limo.SetMotionCommand(1, -0.2)  # 后退1米
    time.sleep(1)  
...
def move_to_backtrack(now_absolute_heading, backtrack_path_coordinates):
    ...
    for i in range(len(backtrack_path_coordinates) - 1):
        ...
        if now_absolute_heading == 0:
            ...
            print(f"Unknown absolute heading: {now_absolute_heading}")
            break
