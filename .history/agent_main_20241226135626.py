...
class NavigationAgent:
    ...
    #     # 保存指令到日志
    #     write_to_record_file(f"导航指令: {instruction}", self.log_file)
    def write_log(self, message):
    ...
    def rollout(self,instruction):
        ...
        for t in range(self.args.max_action_len):
            ...
            # 写入日志
            self.write_log(f"时间步 {t}: 动作 {a_t}，移动到视点 {obs['viewpoint']},是 place: {obs['viewpointId']}, 对应 image {obs['viewpointId']}")
             # 显示进度条
            ...
            # 记录每步时间
            self.timer.toc('step')
            self.write_log(f"Step {t}: 当前状态 {obs['viewpoint']}, 下一步动作 {nav_output}")
            self.timer.step()
            # 记录执行时间
        logger.info(f"步骤 {t} 执行时间: {self.timer.cul['step']:.2f}秒")
