...
class PromptManager(object):
    ...
    def make_image_prompt_json(self): 
        ...
        item_list = """Identify and list all objects observed in each scene. Objects may include tables, chairs, doors, walls, windows, stairs, walkable pathways, or any other items visible. Walkable pathways refer to unobstructed and open ground that allows the robot to navigate forward. 
                    Provide the output as a list in the following format:
                    image4: [table, chair, door, walkable pathway]; 
                    image9: [wall, window, stairs]."""

        # Part 4: Walkable Area Identification 
        walkable_area = """Identify if there are any clear walkable pathways in each scene. 
    ...
    def make_r2r_json_prompts(self, cand_inputs, image_info, obs, t):
        ...
        dist_require = """If you can already see the destination in the descriptions, estimate the distance between you and it. 
                        If the distance is far, continue moving and try to stop within 1 meter of the destination."""
