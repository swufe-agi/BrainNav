...
class PromptManager(object):
    ...
    def make_image_prompt_json(self): 
        ...
        navigable_area = """Identify if there are any clear walkable pathways in each scene. 
                        Walkable pathways refer to unobstructed ground (i.e., no large cardboard boxes) that is open and allows the robot to move forward. 
                        If a scene contains walkable pathways, explicitly indicate it and return a list of all such images. 
                        Example:
                        road: [6, 9] (images 6 and 9 have walkable pathways, with no obstructions).
                        The format `road: [6, 9]` is not allowed; it must not appear as `road: ['image6', 'image9']`."""

        # Part 5: Distance to Obstacles 
        distance_to_obstacles = """Estimate the distance to the nearest obstacle (e.g., wall, cardboard box) in each observed scene.
    ...
    def task_description(self, t):
        ...
        """
        background = """You are an embodied robot that navigates in the real world. Your task is to explore between locations marked with IDs and ultimately find the target location. Once you are within 0.3 meters of the target, you should stop and report your final position.
    ...
    "Action": Select the letter of the next action from the 'Action options', ensuring it corresponds to a node in the 'road' field. Output only the action letter, such as Action: 'A'.Note: For turn around and backtracking, prioritize backtracking."""
