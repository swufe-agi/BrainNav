...
class PromptManager(object):
    ...
    "Thought": Reason about the task by referring to the 'instruction', 'history', 'map', and candidate node descriptions. Prioritize turning left or right to avoid obstacles, and avoid backtracking unless absolutely necessary. Only consider actions for nodes listed in the 'road' field (walkable pathways). If no walkable pathways exist, backtrack only if no other options are available.
