...
class NavigationAgent:
    ...
    def __init__(self, args, prompt_manager=None):
        self.args = args
        self.results = []
...
#     nav_agent.test_backtrack_functionality()
    ...
    def rollout(self):
        ...
        obs = {
            ...
            'instr_id': 0,  # 指令的唯一标识符
            'instruction': "Find the stairwell and stop in front of it.",#导航指令
            'candidate': [  # 候选点信息
                {
        ...
        # print("初始===========================obs['history']:================================",obs['history'])
        ...
        for t in range(self.args.max_action_len):
            ...
            print("--图片分析完成--")
            # print("image_info_output:",image_info_output)
            image_info = json.loads(image_info_output)
            # image_info = image_info_output
            # print(image_info)
            # 处理输出
            # image_result = self.prompt_manager.parse_json_image_info(image_info)
            # print(image_result)
            ...
            # print("self.item_list_all:", self.item_list_all)
            ...
            if self.args.response_format == 'str':
                ...
                # 根据提示生成导航输入（JSON格式）
                nav_input = self.prompt_manager.make_r2r_json_prompts(cand_inputs=cand_inputs,image_info=image_info, obs=obs, t=t)
            else:
                # 如果响应格式不支持，抛出异常
...
if __name__ == "__main__":
    ...
    traj = nav_agent.rollout()  # 调用主导航逻辑
