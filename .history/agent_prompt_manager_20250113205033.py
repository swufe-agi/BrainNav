...
class PromptManager(object):
    ...
    def task_description(self, t):
        ...
        """
        background = """You are an embodied robot that navigates in the real world. Your task is to explore between locations marked with IDs and ultimately find the target location. Once you are within 0.1 meters of the target, you should stop and report your final position.
    ...
    def requirements_and_thought(self):
        ...
        dist_require = """If you can already see the destination in the descriptions, estimate the distance between you and it. 
                                ...
                                You must estimate the distance by referencing the visual proportion of the object or known object dimensions. 
    ...
    "Thought": Reason about the task by referring to the 'instruction', 'history', 'map', and candidate node descriptions. Only consider actions for nodes listed in the 'road' field (walkable pathways).Prioritize turning left or right to avoid obstacles (such as cardboard boxes).
    "New Planning": Update your multi-step navigation plan based on the current step and available pathways.
    "Action": Select the letter of the next action from the 'Action options', ensuring it corresponds to a node in the 'road' field. Output only the action letter, such as Action: 'A'. When choosing Action: 'C', you must prioritize selecting 'F' instead of 'C'."""
